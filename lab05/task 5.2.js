...
//run the function called when any button is clicked
function updateValues(type) {
    ...
    svg.selectAll("rect")
        ...
        }) // stagger the application of a transition based on each type of transition
        ...
        }) // change how long a transition taking place based on each type of transition
        .ease(d3.easeCubicInOut) //types of easing
        .attr("fill", function(d){
            return "hsl(40, 100%, " + (lightness - d) +  "% )"; //color customisation: the orange's lightness is depended on the value in the dataset: the larger the value, the less light the orange
        })
        .attr("y", function(d) {
